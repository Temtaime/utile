...
int mz_zip_reader_init_mem(mz_zip_archive* pZip, in void* pMem, size_t size, uint flags);
int mz_zip_reader_init_file(mz_zip_archive* pZip, in char* pFilename, uint flags);
int mz_zip_reader_init_file_v2(mz_zip_archive* pZip, in char* pFilename,
...
int mz_zip_reader_locate_file(mz_zip_archive* pZip, in char* pName, in char* pComment, uint flags);
int mz_zip_reader_locate_file_v2(mz_zip_archive* pZip, in char* pName,
		in char* pComment, uint flags, uint* mz_file_index);
...
int mz_zip_reader_extract_file_to_callback(mz_zip_archive* pZip,
		in char* pFilename, mz_file_write_func pCallback, void* pOpaque, uint flags);

/*mz_zip_reader_extract_iter_state* mz_zip_reader_extract_iter_new(mz_zip_archive* pZip, uint mz_file_index, uint flags);
mz_zip_reader_extract_iter_state* mz_zip_reader_extract_file_iter_new(mz_zip_archive* pZip, in char* pFilename, uint flags);
...
int mz_zip_reader_extract_to_file(mz_zip_archive* pZip, uint mz_file_index,
		in char* pDst_filename, uint flags);
int mz_zip_reader_extract_file_to_file(mz_zip_archive* pZip,
		in char* pArchive_filename, in char* pDst_filename, uint flags);
...
int mz_zip_reader_extract_file_to_cfile(mz_zip_archive* pZip,
		in char* pArchive_filename, FILE* pFile, uint flags);
...
int mz_zip_validate_mem_archive(in void* pMem, size_t size, uint flags, mz_zip_error* pErr);
int mz_zip_validate_file_archive(in char* pFilename, uint flags, mz_zip_error* pErr);
...
int mz_zip_writer_init_file(mz_zip_archive* pZip, in char* pFilename,
		ulong size_to_reserve_at_beginning);
int mz_zip_writer_init_file_v2(mz_zip_archive* pZip, in char* pFilename,
		ulong size_to_reserve_at_beginning, uint flags);
int mz_zip_writer_init_cfile(mz_zip_archive* pZip, FILE* pFile, uint flags);
int mz_zip_writer_init_from_reader(mz_zip_archive* pZip, in char* pFilename);
int mz_zip_writer_init_from_reader_v2(mz_zip_archive* pZip, in char* pFilename, uint flags);

int mz_zip_writer_add_mem(mz_zip_archive* pZip, in char* pArchive_name,
		in void* pBuf, size_t buf_size, uint level_and_flags);
int mz_zip_writer_add_mem_ex(mz_zip_archive* pZip, in char* pArchive_name, in void* pBuf, size_t buf_size,
		in void* pComment, ushort comment_size, uint level_and_flags,
		ulong uncomp_size, uint uncomp_crc32);
int mz_zip_writer_add_mem_ex_v2(mz_zip_archive* pZip, in char* pArchive_name, in void* pBuf, size_t buf_size,
		in void* pComment, ushort comment_size, uint level_and_flags,
		ulong uncomp_size, uint uncomp_crc32, ulong* last_modified,
		in char* user_extra_data_local,
		uint user_extra_data_local_len, in char* user_extra_data_central,
		uint user_extra_data_central_len);

int mz_zip_writer_add_file(mz_zip_archive* pZip, in char* pArchive_name,
		in char* pSrc_filename, in void* pComment, ushort comment_size, uint level_and_flags);
int mz_zip_writer_add_cfile(mz_zip_archive* pZip, in char* pArchive_name, FILE* pSrc_file, ulong size_to_add,
		in ulong* pFile_time, in void* pComment, ushort comment_size, uint level_and_flags, in char* user_extra_data_local,
		uint user_extra_data_local_len, in char* user_extra_data_central,
		uint user_extra_data_central_len);
...
int mz_zip_add_mem_to_archive_file_in_place(in char* pZip_filename, in char* pArchive_name,
		in void* pBuf, size_t buf_size, in void* pComment, ushort comment_size, uint level_and_flags);
int mz_zip_add_mem_to_archive_file_in_place_v2(in char* pZip_filename, in char* pArchive_name, in void* pBuf,
		size_t buf_size, in void* pComment, ushort comment_size,
		uint level_and_flags, mz_zip_error* pErr);

void* mz_zip_extract_archive_file_to_heap(in char* pZip_filename,
		in char* pArchive_name, size_t* pSize, uint flags);
void* mz_zip_extract_archive_file_to_heap_v2(in char* pZip_filename,
		in char* pArchive_name, in char* pComment, size_t* pSize, uint flags, mz_zip_error* pErr);
